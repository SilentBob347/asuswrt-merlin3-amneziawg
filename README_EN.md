...
- **The double hop while Xray runs — via the «bypass Xray» checkbox**: Xray grabs traffic ahead of AmneziaWG's rules (ip-rule 19 vs 99), so a peer with a VPN policy **without** that checkbox falls through to Xray instead of the client tunnel — the page flags this with a red banner. Options: turn on «bypass Xray» for the peer (double hop on top of a running Xray, see below), or leave the peer on "Direct" (it flows through Xray), or stop Xray with the button in the banner. (For LAN devices this priority is automatic — the `AWG_PRIO` chain, 1.3.12; for peers it is deliberately left to the «bypass Xray» toggle.)
...
- **«Bypass Xray» option (per-peer, off by default)**: if you want the actual double hop through the client tunnel rather than proxying via Xray — a peer with a VPN policy gets a «bypass Xray» checkbox. It inserts a rule ahead of Xray's capture, steering that peer's traffic into awg0 (the client tunnel must be up). Other peers keep going through Xray. Xray keeps running and is not reconfigured.
...
Important: this only removes the DNS-level clash. With the default policy **"VPN -- all traffic"** the routing still steals traffic from the co-resident proxy, so for coexistence pick **"Direct"** or **"VPN -- Geo only"**, not "all traffic". Geo-by-IP keeps working either way. That's about the **default** policy — individual devices can still be assigned to AmneziaWG, and since 1.3.12 they get priority into the tunnel even while Xray is active (see the next item).

**The reverse case — XRAYUI and AmneziaWG's traffic.** If XRAYUI runs in transparent-proxy "redirect all traffic" mode (TPROXY), it captures LAN traffic in mangle PREROUTING ahead of AmneziaWG's rules (ip-rule 19 vs 98). This used to mean a device you assigned to `VPN: all traffic` or `VPN: Geo only` actually went out through Xray, not the tunnel. **Since 1.3.12 this is auto-resolved:** while Xray is capturing, the addon stands up its own `AWG_PRIO` chain at the very top of PREROUTING (before XRAYUI) and gives AmneziaWG-assigned devices priority into the tunnel (except an exclude-mode "Direct by lists" geo policy, which stays with Xray); the rest of the LAN keeps using Xray. Xray's own rules are untouched. The page shows a yellow informational banner explaining this. **One case the priority chain can't fix:** if XRAYUI also captures the router's own egress, including AmneziaWG's handshake, the tunnel comes up but passes no traffic (the health check rolls it back after ~60 s) — then exclude the endpoint and the `awg0` interface from XRAYUI's capture, or stop Xray (button in the banner), or run only one VPN at a time.
...
A: Check the banners on the server page. Common causes: the client tunnel is not running (a peer's "VPN" policy is the double hop through it; while it's down, traffic goes direct — fail-open), or Xray/XRAYUI runs in "redirect all" mode — then Xray grabs a VPN-policy peer's traffic ahead of the addon's rules. To still get the double hop on top of a running Xray, turn on that peer's **«bypass Xray»** checkbox (the client tunnel must be up); or leave the peer on "Direct" — its traffic flows through Xray anyway (and the page will tell you if Xray hasn't picked up the peer subnet — fixed by restarting XRAYUI).
